...
// Stopword list — common English words to filter out
...
function extractNgrams(text: string): string[] {
  ...
  // Normalize: lowercase, remove punctuation except hyphens between words
...
serve(async (req: Request) => {
  ...
  if (job_id) {
    ...
    // Fallback: try by other id fields if needed
    if (!jdText) {
  ...
  if (resume_id) {
    ...
    if (!resumeText) {
      const { data: res2 } = await sb
        .from("resumes")
        .select("extracted_text, keywords")
        .eq("id", resume_id)
        .maybeSingle();
    ...
    await fetch("https://app.posthog.com/capture/", {
      ...
      body: JSON.stringify({
        ...
        properties: {
          ...
          jd_ngram_count: jdNgrams.length,
          resume_ngram_count: resumeNgrams.length,
          gap_term_count: gapTerms.length,
          has_jd_text: jdText.length > 0,
          has_resume_text: resumeText.length > 0,
        },
      }),
    });
  } catch { /* non-fatal */ }
