...
serve(async (req) => {
  ...
  try {
    ...
    const credit_rewrite_resume_analyze = await creditGate(sb, user.id, 'rewrite-resume-analyze');
    if (!credit_rewrite_resume_analyze.allowed) return credit_rewrite_resume_analyze.response!;
