-- 20260323000006: Force correct volatility on search_jobs
-- DROP required because ALTER FUNCTION can't change volatility retroactively
-- and CREATE OR REPLACE doesn't change existing volatility

DROP FUNCTION IF EXISTS search_jobs(uuid, uuid[], integer, integer, text, boolean);

-- Recreate with correct VOLATILE declaration
