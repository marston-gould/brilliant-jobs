// Enhanced Form Detector for Indeed
// Handles dynamic loading, multiple selectors, and intelligent waiting

export class GlassdoorFormDetector {
  constructor() {
  ...
  setupDetectionStrategies() {
    this.detectionStrategies = [
      ...
      // Strategy 4: Radio/Checkbox containers
      {
        name: 'input_containers',
        selector: 'div:has(input[type="radio"]), div:has(input[type="checkbox"])',
        questionSelectors: [
  ...
  isLikelyQuestion(element) {
    ...
    // Should have some text content (question text)
    ...
    const questionIndicators = [
      ...
      'disability', 'orientation', 'identity', 'background'
    ];
  ...
  calculateConfidence(form, questions, strategy) {
    ...
    // Boost confidence based on question count
    if (questions.length >= 3) confidence += 0.1;
    if (questions.length >= 5) confidence += 0.1;

    // Boost confidence for demographic keywords
    ...
    // Boost confidence for proper form structure
  ...
  async waitForQuestionsToLoad(form, maxWaitTime = 10000) {
    ...
    console.log("⏳ Waiting for questions to load completely...");
    ...
    while (Date.now() - startTime < maxWaitTime) {
      ...
      if (currentQuestionCount === lastQuestionCount) {
        stableCount++;
        // If count is stable for 3 checks and we have questions, we're good
        if (stableCount >= 3 && currentQuestionCount > 0) {
          console.log(`✅ Questions loaded: ${currentQuestionCount} questions detected`);
          return true;
        }
      } else {
        stableCount = 0;
        lastQuestionCount = currentQuestionCount;
        console.log(`📊 Questions loading: ${currentQuestionCount} detected so far...`);
      }

      await this.delay(500);
    }

    console.log(`⚠️ Question loading timeout: ${lastQuestionCount} questions found`);
    return lastQuestionCount > 0; // Return true if we have at least some questions
  }
