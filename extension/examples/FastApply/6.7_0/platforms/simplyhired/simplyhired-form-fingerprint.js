// Form Fingerprinting System for SimplyHired
export class FormFingerprinter {
  constructor() {
  ...
  getFormClasses(questionsForm) {
    return {
      ...
      questionItemClasses: Array.from(questionsForm.querySelectorAll('[class*="Questions"], [class*="questions"]'))
        .map(el => el.className)
        .slice(0, 5), // First 5 for performance
  ...
  analyzeQuestionStructure(questionsForm) {
    ...
    const checkboxInputs = questionsForm.querySelectorAll('input[type="checkbox"]');
    const checkboxGroups = new Set();
    checkboxInputs.forEach(checkbox => {
      if (checkbox.name) checkboxGroups.add(checkbox.name);
    });
    structure.checkboxGroups = Array.from(checkboxGroups);

    // Analyze select fields
    const selectFields = questionsForm.querySelectorAll('select');
    structure.selectFields = Array.from(selectFields).map(select => ({
      id: select.id,
      name: select.name,
      options: select.options.length,
      required: select.hasAttribute('required') || select.hasAttribute('aria-required')
    }));

    // Analyze text fields
    const textFields = questionsForm.querySelectorAll('input[type="text"], textarea, input[type="email"], input[type="tel"]');
    structure.textFields = Array.from(textFields).map(field => ({
      id: field.id,
      name: field.name,
      type: field.type,
    ...
    const fileFields = questionsForm.querySelectorAll('input[type="file"]');
    structure.fileUploads = Array.from(fileFields).map(field => ({
      id: field.id,
      name: field.name,
      required: field.hasAttribute('required') || field.hasAttribute('aria-required')
    }));
