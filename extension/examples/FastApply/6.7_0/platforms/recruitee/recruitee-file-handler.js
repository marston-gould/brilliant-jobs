...
export class RecruiteeFileHandler {
  ...
  isElementVisible(element) {
    ...
    // File inputs with opacity:0 are a common UX pattern (visually hidden but accessible)
  ...
  async generateAndUploadCoverLetter(fileInput, userDetails, jobDescription) {
    try {
      ...
      if (contentType && !contentType.includes("application/pdf")) {
        console.warn("⚠️ Expected PDF but received:", contentType);
      }
      ...
      console.log("⬆️ Uploading blob to file input...");
  ...
  determineFileType(fileInput) {
    try {
      ...
      const dataCy = (fileInput.getAttribute("data-cy") || "").toLowerCase();

      console.log("🔍 Determining file type:", { name, id, dataQa, dataCy });
      ...
      if (
        ...
        dataQa.includes("coverletter") ||
        dataQa.includes("cover") ||
        dataCy.includes("coverletter") ||
        dataCy.includes("cover")
      ) {
        console.log(
          "✅ Detected as coverLetter (from name/id/data attributes)"
        );
        return "coverLetter";
      }
      ...
      if (
        ...
        dataCy.includes("resume")
      ) {
        console.log("✅ Detected as resume (from name/id/data attributes)");
        return "resume";
      }
      ...
      if (container) {
        ...
        // Check for cover letter in container (more specific)
        if (
          containerText.includes("cover letter") ||
          containerText.includes("coverletter")
        ) {
          console.log("✅ Detected as coverLetter (from container text)");
          return "coverLetter";
        }

        if (containerText.includes("resume") || containerText.includes("cv")) {
          console.log("✅ Detected as resume (from container text)");
          return "resume";
        }
      }
      ...
      if (label) {
        ...
        // Check for cover letter in label (more specific)
        if (
          labelText.includes("cover letter") ||
          labelText.includes("coverletter")
        ) {
          console.log("✅ Detected as coverLetter (from label)");
          return "coverLetter";
        }

        if (labelText.includes("resume") || labelText.includes("cv")) {
          console.log("✅ Detected as resume (from label)");
          return "resume";
        }
      }

      console.log("⚠️ Defaulting to resume");
      return "resume";
    } catch (error) {
