...
describe('AIS-F4-S1: bj:toolbar:answerReviewConfirm handler', () => {
  ...
  it('logs submission attempt on confirm', () => {
    expect(src).toMatch(/answerReviewConfirm[\s\S]{0,2500}_logSubmissionAttempt/);
  });

  it('catches errors — no silent fail', () => {
    expect(src).toMatch(/answerReviewConfirm[\s\S]{0,2500}extension_catch_error/);
  });
});
...
describe('AIS-F4-S1: window action handlers in overlay', () => {
  ...
  it('_bjAnswerReviewAccept collects edited textarea values', () => {
    expect(src).toMatch(/window\._bjAnswerReviewAccept[\s\S]{0,600}ta\.value/);
  });
...
describe('AIS-F4-S1: contentScript.ts handlers', () => {
  ...
  it('skips standard fields via regex pattern', () => {
    expect(src).toMatch(/collectQuestions[\s\S]{0,2500}standardPat/);
  });

  it('caps questions at 10', () => {
    expect(src).toMatch(/collectQuestions[\s\S]{0,2500}slice\(0,\s*10\)/);
  });

  it('responds with questions array', () => {
    expect(src).toMatch(/collectQuestions[\s\S]{0,2500}sendResponse.*questions/);
  });

  it('catches errors returning empty array', () => {
    expect(src).toMatch(/collectQuestions[\s\S]{0,3000}catch[\s\S]{0,300}questions.*\[\]/);
  });
...
// ─────────────────────────────────────────────────
// 14. aiAnswerer.ts: tier gate (AIS-F4-S1 gap fix)
// ─────────────────────────────────────────────────
describe('AIS-F4-S1: aiAnswerer.ts tier gate', () => {
  const src = read('extension/utils/aiAnswerer.ts');

  it('tier gate check exists in answerCustomQuestions', () => {
    expect(src).toContain('AIS-F4-S1: Tier gate');
  });

  it('free tier is blocked', () => {
    expect(src).toContain("|| 'free'");
    expect(src).toContain('isEligible');
  });

  it('starter tier is allowed', () => {
    expect(src).toContain("userRole === 'starter'");
  });

  it('pro tier is allowed', () => {
    expect(src).toContain("userRole === 'pro'");
  });

  it('admin tier is allowed', () => {
    expect(src).toContain("userRole === 'admin'");
  });

  it('returns tier_blocked confidence for free users', () => {
    expect(src).toContain("confidence: 'tier_blocked'");
  });
  ...
  it('storage check failure is non-fatal (proceeds)', () => {
    expect(src).toMatch(/catch[\s\S]{0,50}\/\/ Non-fatal/);
  });
});
