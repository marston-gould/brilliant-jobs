    ...
    <div className="flex flex-col items-center gap-2 py-4 border-t border-border">
      ...
      {totalPages > 1 && (
        <div className="flex items-center gap-1 flex-wrap justify-center">
          <button
            className="min-w-[32px] h-8 px-2 border border-border rounded-md bg-bg-card text-text-dim text-xs font-semibold font-sans cursor-pointer transition-all inline-flex items-center justify-center hover:border-accent hover:text-accent hover:bg-accent-dim disabled:opacity-30 disabled:cursor-default"
            disabled={page === 0}
          ...
          {pages.map((p, i) =>
            p === '...' ? (
              <span key={`ellipsis-${i}`} className="w-6 text-center text-text-faint text-xs inline-flex items-center justify-center">…</span>
            ) : (
              <button
                key={p}
                className={`min-w-[32px] h-8 px-2 border rounded-md text-xs font-semibold font-sans cursor-pointer transition-all inline-flex items-center justify-center ${
                  p === page
                    ? 'bg-accent text-white border-accent cursor-default'
                    : 'border-border bg-bg-card text-text-dim hover:border-accent hover:text-accent hover:bg-accent-dim'
