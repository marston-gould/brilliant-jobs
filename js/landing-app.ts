...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (previewGoBtn) {
      previewGoBtn.addEventListener('click', async () => {
        ...
        try {
          ...
          function pvCard(job) {
            var level = pvGetLevel(job.title);
            ...
            var sal = pvSalary(job.salary_min, job.salary_max);
            if (sal) meta.push('<span>' + sal + '</span>');
            var days = pvDays(job.first_seen_at);
            if (days) meta.push('<span>' + days + '</span>');
            return '<div class="pv-job-card"><div class="pv-card-body"><div class="pv-title-row"><span class="pv-job-title">' + DOMPurify.sanitize(job.title) + '</span>' + badgeHtml + '</div><div class="pv-meta-row">' + meta.join('<span class="pv-meta-dot">·</span>') + '</div></div><div><button class="pv-signup-btn">Sign Up</button></div></div>';
          }
    ...
    // Preview signup buttons — event delegation for dynamic cards (FB-PREVIEW-CARDS-001)
...
(function initLpBenefitSections() {
  ...
  async function loadBenefitSections() {
    ...
    try {
      ...
      try { if (typeof reportError === 'function') reportError('lp_benefit_sections', e); } catch(_) {}
    }
  }
