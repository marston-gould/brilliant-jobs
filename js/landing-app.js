document.addEventListener("DOMContentLoaded", function() {
  ...
  window.closeModal = function closeModal2() {
    var modal = $("#auth-modal");
  ...
  function switchTab(form) {
    ...
    if (form === "forgot") {
      $$(".auth-tabs")[0].style.display = "none";
    } else {
      $$(".auth-tabs")[0].style.display = "";
    }
  ...
  $("#nav-login-btn").addEventListener("click", (e) => {
    e.preventDefault();
    openModal(isReturning ? "login" : "signup");
  });
  ...
  if (lapsedLogin) lapsedLogin.addEventListener("click", (e) => {
    e.preventDefault();
    openModal("login");
  });
  ...
  $("#login-btn").addEventListener("click", async () => {
    ...
    msgEl.style.display = "none";
    if (!email || !password) {
      showMsg("login-msg", "Fill in all fields.", "error");
      return;
    }
  ...
  if (signupBtn) signupBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (!fullName) {
        showMsg("signup-msg", "Please enter your full name.", "error");
        return;
      }
      if (!email || !password) {
        showMsg("signup-msg", "Fill in all fields.", "error");
        return;
      }
      ...
      var elapsedSeconds = signupStartedAt ? Math.round((Date.now() - signupStartedAt) / 1e3) : 0;
  ...
  async function linkReferral(userId, code, source) {
    try {
      await fetch(SUPABASE_URL + "/functions/v1/referral-lifecycle", {
        method: "POST",
        headers: { "Content-Type": "application/json", "apikey": SUPABASE_KEY },
        body: JSON.stringify({ type: "referee_signup", referee_id: userId, metadata: { referral_code: code, source } })
      });
...
(function initLpBenefitSections() {
  ...
  function sanitizeBodyText(raw) {
    ...
    var md = raw.replace(/\*\*(.+?)\*\*/g, "<strong>$1</strong>").replace(/\[([^\]]+)\]\((https?:\/\/[^)]+)\)/g, '<a href="$2" rel="noopener">$1</a>');
  ...
  function renderSection(section, position) {
    ...
    var imgHtml = hasImage ? '<div class="lp-section-img-wrap"><div class="lp-section-browser-frame"><div class="lp-section-browser-dots"><span></span><span></span><span></span></div><img src="' + escapeAttr(section.image_url) + '" alt="' + escapeAttr(section.image_alt || section.title) + '" loading="lazy" class="lp-section-screenshot"></div></div>' : '<div class="lp-section-img-wrap lp-section-img-placeholder"><div class="lp-section-browser-frame lp-section-browser-frame--empty"><div class="lp-section-browser-dots"><span></span><span></span><span></span></div><div class="lp-section-placeholder-inner">Screenshot coming soon</div></div></div>';
    var ctaHtml = section.cta_text && section.cta_url ? '<a href="' + escapeAttr(section.cta_url) + '" class="btn btn-primary lp-section-cta">' + escapeHtml(section.cta_text) + "</a>" : "";
    var bodyParagraphs = sanitizeBodyText(section.body_text).split(/\n\n+/).filter(Boolean).map(function(p) {
      return "<p>" + p + "</p>";
    }).join("");
    var contentHtml = '<div class="lp-section-content">' + (section.subtitle ? '<div class="lp-section-subtitle">' + escapeHtml(section.subtitle) + "</div>" : "") + '<h2 class="lp-section-title">' + escapeHtml(section.title) + '</h2><div class="lp-section-body">' + bodyParagraphs + "</div>" + ctaHtml + "</div>";
    var el = document.createElement("section");
    el.className = "lp-benefit-section section fade-up " + orientClass;
    el.setAttribute("data-section-id", section.id);
    el.innerHTML = orientClass === "section-img-left" ? imgHtml + contentHtml : contentHtml + imgHtml;
    return el;
  }
  ...
  async function loadBenefitSections() {
    ...
    if (!container) return;
    try {
      var url = SUPABASE_URL + "/rest/v1/landing_sections?is_visible=eq.true&archived_at=is.null&order=sort_order.asc";
      ...
      try {
        if (typeof reportError === "function") reportError("lp_benefit_sections", e);
      } catch (_) {
      }
    }
  }
