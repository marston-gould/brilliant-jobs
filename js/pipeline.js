    ...
    for (const item of jobs) {
      ...
      if (stage === 'interview') {
        ...
        html += '<td style="white-space:nowrap;">' + _readinessBadge +
          '<button onclick="event.stopPropagation();if(window.openInterviewPractice)window.openInterviewPractice(\'' + item.id + '\',\'' + title.replace(/\'/g, \'\\\'\')+  '\',\'' + company.replace(/\'/g, \'\\\'\')+  '\')" style="display:inline-block;font-size:10px;font-weight:600;padding:4px 10px;border-radius:6px;background:var(--purple,#7c3aed);color:#fff;border:none;cursor:pointer;white-space:nowrap;margin-right:4px;" title="AI Interview Practice (AIS-F11)">🎯 Practice</button>' +
