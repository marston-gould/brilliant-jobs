...
| POD3-SF | Saved Filters UX Fixes — 3 issues. (1) Removed 1D/7D/30D column headers and per-row counts from saved filters list. (2) Pill changes now auto-save to the saved filter being edited (via _editingFilterIdx) and re-run query immediately. debouncedSearchJobs() called unconditionally. Clear All clears editing state. (3) Saved filter search now matches pill values (what/where/when/who/pay/skills/level/jd/dept) in addition to filter names. | 3h | ✅ | All 3 issues resolved. 21 validation tests. v7.81. 2026-03-08. |
