...
| CASA-001 | **PENDING: Google CASA assessment + gmail.readonly upgrade.** AI classifier test on 12 ambiguous real emails showed 58% misclassification without snippet (7/12 wrong — 4 rejections classified as ACK, 3 signals dropped as NONE). Subject-only fails on vague subjects like "Application Status Update", "Your application at Rasa", "Regarding your application". Snippet provides the rejection/interview language the classifier needs. **Action items:** (1) Budget $500–$4,500 for CASA third-party security assessment (annual). (2) Submit Google Cloud Console for verification with gmail.readonly scope. (3) After CASA passes: revert gmail-auth scope to gmail.readonly, redeploy. (4) Existing gmail.metadata users re-authenticate to get snippet access. **Blocked on:** Marston budget approval + CASA vendor selection. | ⏳ | Test data: 77 real emails, 12 ambiguous, 7 AI misclassified without snippet. docs/google-oauth-verification-prep.md has full checklist. |
...
| EXT-BUILD-001-S2 | Dashboard Download Button + Version Check — Wire download button, extension-version EF, background.ts version check alarm, popup update banner | 🔲 | Depends on S1 |
| EXT-BUILD-001-S3 | CI Gate + Release Process — gate-ext-build CI job, release docs, build-extension.js three-mode fix, test suite | 🔲 | Depends on S2 |
