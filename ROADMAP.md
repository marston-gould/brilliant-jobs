...
<<<<<<< HEAD
| AIS-F4-S1 | AI Q&A Gate Removal + Answer Review Mode — Remove admin flag from aiAnswerer.js + answer-form-question EF. Wire tier gate. Add pre-submit answer review panel for Score-Gated/Manual modes (edit, accept, or regenerate before submit). PostHog: ai_answer_feedback (thumbs up/down). | ✅ | v9.56 — _fetchAiAnswersForReview in background.ts, showAnswerReviewPanel in overlay, collectQuestions in contentScript, ai_answer_generated + ai_answer_feedback PostHog events. 59 tests. v9.57 gap fixes: answers table migration, DB answer cache, persistAnswers, deductCredits (0.5/answer cached=free), LinkedIn profile context in prompt, cache_hits + credits_charged in response. 50 additional tests. |
=======
...
| ID | Item | Status | Notes |
|----|------|--------|-------|
| AIS-F8-S1 | Cover Letter Generator — UI + Table — cover_letters table (user_id, job_id, resume_id, tone, content, version, ai_score, credits_charged, created_at). Slide-out panel on Applications page. Tone selector (Professional/Conversational/Enthusiastic/Executive). Version history (each regeneration = new version). DOCX export. Wire existing generate-cover-letter EF for consumer access (remove admin gate). 2 credits/letter. PostHog: cover_letter_generated. | 🔲 | |
| AIS-F8-S2 | Cover Letter Auto-Attach in Apply Flow — Auto-generate cover letter at apply time if user has setting enabled. Detect cover letter fields in ATS forms, paste content. Cache for same company (reuse on re-apply). Wire into auto-apply and bulk apply flows. | 🔲 | |
...
| AIS-F1-S4 | Resume Tailoring — CTA Triggers + Credit System — Credit balance infrastructure: check balance, deduct on success, top-up flow. 3 credits deducted on successful rewrite, 0 on failure. CTA trigger points: Jobs Feed Match% column (<85%), Resume Readiness grade card (<A), Job Detail slide-out, Pipeline Saved stage. PostHog: resume_rewrite_completed. | 🔲 | |
...
| AIS-F6-S1 | Review Before Submit — Interception Panel — Pre-submit review panel (Shadow DOM or slide-out): job title, company, match score, resume version, AI-generated answers (editable inline), cover letter (if exists, regeneratable). Submit / Cancel / Save for Later actions. Submit fires auto-fill. Save for Later queues to Review Queue. PostHog: review_panel_shown. | 🔲 | |
| AIS-F6-S2 | Review Queue on Dashboard — Review Queue section on Applications page: jobs parked via Save for Later. Process from desktop. Per-job status indicators. Integration with pipeline board (parked jobs tracked as Saved stage). | 🔲 | |
...
| ID | Item | Status | Notes |
|----|------|--------|-------|
| AIS-F8-S1 | Cover Letter Generator — UI + Table — cover_letters table. Slide-out panel on Applications page. Tone selector (4 tones). Version history. DOCX export. Wire generate-cover-letter EF for consumers. Credit: 2/letter. PostHog: cover_letter_generated. | 🔲 | Pod 2: 1d, Pod 3: 4d |
| AIS-F8-S2 | Cover Letter Auto-Attach in Apply Flow — Attach cover letter in auto-apply/bulk-apply flows. ATS form cover letter field detection. Cache: same company reuses letter. Auto-generate option in mode settings (2 credits). | 🔲 | Pod 2: 2d |
| AIS-F1-S1 | Resume Tailoring — EF Agents 1–2 (Gap Analyzer + Question Generator) — Claude Haiku. Structured gap list + 1–5 targeted questions. resume_rewrites table. | 🔲 | Pod 2: 3d |
| AIS-F1-S2 | Resume Tailoring — EF Agents 3–4 (Rewriter + Quality Checker) — Claude Sonnet rewriter. Fabrication detection (>95% truthfulness gate). Diff JSON output. Status: pending→processing→complete/failed. | 🔲 | Pod 2: 3d |
| AIS-F1-S3 | Resume Tailoring — Q&A Panel + Diff Preview UI — Client-side Q&A panel (progress indicator, skip/back, career coach tone). Side-by-side diff (green/amber/red). Accept all / cherry-pick / reject with feedback. DOCX download. | 🔲 | Pod 3: 3d |
| AIS-F1-S4 | Resume Tailoring — Credit System + CTA Triggers — Credit infrastructure (check + deduct + top-up). 3 credits on success, 0 on failure. CTA triggers: Feed Match% below 85%, Resume Readiness below A, Job Detail slide-out, Pipeline Saved stage. PostHog: resume_rewrite_started, resume_rewrite_completed, resume_rewrite_qa_skipped. | 🔲 | Pod 2: 2d, Pod 3: 2d |
...
| AIS-F6-S1 | Review Before Submit — Interception Panel — Pre-submit panel: job title, company, score, resume version, AI answers (editable), cover letter preview. Edit-in-place. Submit / Cancel / Save for Later. PostHog: review_panel_shown. | 🔲 | Pod 2: 1d, Pod 3: 3d |
| AIS-F6-S2 | Review Queue on Dashboard — 'Review Queue' section on Applications page. Jobs parked from Save for Later. Process from desktop. Pipeline integration. | 🔲 | Pod 3: 1d |

### Phase D — Scale + New (Weeks 7–11)

| ID | Item | Status | Notes |
|----|------|--------|-------|
| AIS-F9-S1 | Bulk Apply — Multi-Select UI + Action Bar — Checkbox column on Jobs Feed. 'Select All Matching'. Bulk action bar (apply/save/cover letters). Selected count badge + estimated credit cost. | 🔲 | Pod 3: 2d |
| AIS-F9-S2 | Bulk Apply — Queue Table + EF — bulk_apply_jobs table. bulk-apply-queue EF: sequential processing, 45–90s delay, max 25/session, retry logic (max 2). Score gate integration per-job in batch. | 🔲 | Pod 2: 5d |
| AIS-F9-S3 | Bulk Apply — Progress Dashboard + Safety Controls — Real-time progress bar. Per-job status indicators. Daily limits (Pro=50, Starter=10). Duplicate detection. 30-min cool-down. 10s undo window. PostHog: bulk_apply_started, bulk_apply_completed. | 🔲 | Pod 3: 4d |
| AIS-F10-S1 | LinkedIn Auto-Apply Hardening — Randomized interaction delays. Viewport-aware interactions. Session cookie management. Max 15 Easy Apply/day. CAPTCHA detection + pause + user alert. | 🔲 | Pod 2: 3d |
| AIS-F10-S2 | LinkedIn Multi-Step + Profile Sync — Multi-step Easy Apply (1–6 pages). LinkedIn Q&A optimization. Profile data sync from F2 import. Connection awareness prompt. PostHog: linkedin_easy_apply_triggered. | 🔲 | Pod 2: 1d, Pod 3: 2d |
| AIS-F7-S1 | Resume Builder — Input Wizard + Generation EF — 4–6 screen wizard (role/industry/exp/accomplishments/skills/education). Pre-fill from LinkedIn profile. resume-generate EF (Claude Sonnet, structured sections). | 🔲 | Pod 2: 4d |
| AIS-F7-S2 | Resume Builder — Templates + Editor + Export — 3–5 ATS-friendly templates (CSS-driven, no graphics/columns). Live score preview. Section editor with re-score. DOCX + PDF export. Tier gate: Free=1, Pro=unlimited. PostHog: resume_built_from_scratch. | 🔲 | Pod 2: 2d, Pod 3: 8d |
| AIS-F11-S1 | Interview Practice — EF + Session Table — interview-practice EF (session_type, job_id, resume_id → questions + per-answer feedback). Claude Sonnet. 5-dimension scoring. interview_sessions table. | 🔲 | Pod 2: 5d |
| AIS-F11-S2 | Interview Practice — Chat UI + Feedback + History — Chat UI on Pipeline page (contextual per job). Per-answer feedback. Aggregate scorecard. Session history + improvement tracking. Pipeline 'Interview' stage CTA. Tier gate: Free=1, Pro=unlimited. PostHog: interview_practice_started, interview_practice_completed. | 🔲 | Pod 3: 6d |
| AIS-F12-S1 | Resume A/B Testing — Engine + Tables — resume_ab_tests + resume_ab_results tables. Round-robin assignment logic hooked into apply flows. Pipeline stage outcome tracking. Tier gate: Free=none, Pro/PAYL=1 active test. | 🔲 | Pod 2: 4d |
