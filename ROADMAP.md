...
| COHORT-S2 | EF Layer — get-user-balance EF, debit middleware (auth→cost→balance check→debit→execute→refund_restore), replenish-credits EF (all 3 rollover modes), award-expiry cron. Wire debit middleware into all 8 active-debit EFs + 3 passive EFs with daily cap logic. | 🔲 | — |
| COHORT-S3 | Stripe + UI — customer.subscription.updated webhook → cohort reassign + prorate entry. award-grant EF for referral/promo/milestone sources. Balance breakdown UI on subscription panel (3 buckets, reset date, low-balance warning at 20%). | 🔲 | — |
