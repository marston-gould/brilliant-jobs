...
| RESUME-BUILDER-001-S1 | Phase 1: Upload, Parse, Store — resume-parse EF, resumes table + RLS + Storage bucket, upload UI on page-resume-builder, editable parsed data form, paste-text alternative. | 🔲 | Target: v9.36+. |
| RESUME-BUILDER-001-S2 | Phase 2: Templates & Generation — 3 ATS-compliant .docx templates (Classic/Modern Professional/Clean Minimal), resume-generate EF, template selector UI + live preview, .docx and .pdf download. | 🔲 | Target: v9.37+. |
| RESUME-BUILDER-001-S3 | Phase 3: Keyword Optimization — resume-optimize EF, gap report UI (present/missing/partial), one-click keyword insertion, match score real-time update, "Optimize for this job" entry on job cards. | 🔲 | Target: v9.38+. |
| RESUME-BUILDER-001-S4 | Phase 4: AI Rewrites — resume-rewrite-bullet EF, per-bullet Improve button, accept/reject/edit UI, credit consumption integration. | 🔲 | Target: v9.39+. |
