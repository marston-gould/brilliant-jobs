...
| ID | Item | Status | Notes |
|----|------|--------|-------|
| HOTFIX-MERGE-001 | Removed unresolved git merge conflict markers (=======, >>>>>>>) from 9 HTML files (dashboard.html, index.html, admin.html, compare.html, ghost-report.html, hiring-trends.html, career-level-data.html, data-lab.html, jobs-by-industry.html) left by botched revert of unauthorized mobile-responsive changes. Removed duplicate script/link blocks from dashboard.html (shell+feed+inline doubled), index.html (version+cookie+posthog+flags+logger+ab doubled), admin.html (version+posthog+perf+flags+logger+insights+ab+crewai+echarts doubled). All 15 HTML surfaces re-synced at v9.85. | ✅ | v9.85 |
