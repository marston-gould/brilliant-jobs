...
| FB-PI-001-S5 | Staleness Engine — check-pipeline-staleness cron, staleness prompt cards, auto-archive logic, snooze, backward stage movement, undo | 🔲 | Depends on S3 (parallel with S4) |
| FB-PI-001-S6 | Settings + Polish + Tests — dashboard.html: pi-auto-move-behavior selector (aggressive/conservative/manual), pi-staleness-days range slider (3–30), pi-auto-archive toggle, pi-notify-automove controls. applications.js: savePipelineIntelligenceSettings extended (auto_move_behavior, staleness_threshold_days, auto_archive_enabled, auto_archive_days), loadPipelineTrackingSettings extended, wireStalnessSlider live label. auto_move_behavior column added to pipeline_tracking_settings. Comprehensive cross-session test suite (12 sections, S1–S5 smoke tests, hook/scar audit, file inventory). | ✅ | 82 validation tests. v9.14. 2026-03-15. FB-PI-001 FEATURE COMPLETE. |
