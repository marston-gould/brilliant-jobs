...
| POD3-SF | Saved Filters UX Fixes — 3 issues. (1) Removed 1D/7D/30D column headers and per-row counts from saved filters list. (2) commitSaveFilter now preserves checkbox state across renderSavedFilters DOM rebuild, calls invalidateCache() to bust query cache, uses _editingFilterIdx as primary lookup (name match fallback), calls searchJobs(0) for immediate re-search. Reverted auto-save (was not the issue). (3) Saved filter search now matches pill values (what/where/when/who/pay/skills/level/jd/dept) in addition to filter names. | 3h | ✅ | All 3 issues resolved. 22 validation tests. v7.82. 2026-03-08. |
