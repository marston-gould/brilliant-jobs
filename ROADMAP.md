...
| ID | Item | Status | Notes |
|----|------|--------|-------|
| HOTFIX-MERGE-001 | (v9.85) Removed unresolved git merge conflict markers from 9 HTML files + duplicate script blocks. (v9.86) Fixed AI Interview Practice panel stuck open (display:none;...display:flex; duplicate CSS property). Moved Global Rules + Generate Prompt banners above Filter/Chat toggle. Fixed saved searches/prompts resurrecting after deletion — syncHealthCheck() and loadUserData() both treated empty arrays [] as "missing data" and restored old values from cloud, undoing intentional deletions. Fixed: empty arrays are now treated as valid state, only truly absent keys trigger cloud recovery. | ✅ | v9.86 |
