...
| PC-002 | JS Cleanup — Consolidate pipeline rendering logic, remove switchAppView remnants, ensure hero stat cards navigate correctly, update nav pulse logic | 🔲 | |
| PC-003 | Deep Link + Testing — Verify all deep links, cross-tab navigation, end-to-end functional validation | 🔲 | |
| PC-004 | Version Bump + Deploy — Final deploy, production verification | 🔲 | |
...
| FB-TRIAL-001-S2 | Trial Gate Server — Add checkFeatureAccess gating calls to all 7 gated Edge Functions (chat-job-search, resume-score, send-notification, auto-apply-submit, handle-sms-reply, stats-query, saved-filters). pg_cron trial-expiry-checker. Stripe webhook modifications for state transitions (checkout.session.completed → active_pro, customer.subscription.deleted → expired_free + sample reset). | 🔲 | |
| FB-TRIAL-001-S3 | Trial Gate Client + Free Samples — Trial countdown banner (blue/amber/red by day), pre-sample prompt, post-sample conversion modal, X-Is-Sample header handling, trial-gate.js. | 🔲 | |
| FB-TRIAL-001-S4 | Referral Program — handle-referral-signup + process-referral-reward Edge Functions, Stripe coupon logic, referral clawback pg_cron, cookie handling, referral.js, referral UI surfaces. | 🔲 | |
| FB-TRIAL-001-S5 | Notifications — 12 email templates, send-trial-notifications EF, weekly-digest-expired EF, sample_reminder logic, notification consolidation. | 🔲 | |
| FB-TRIAL-001-S6 | Cost Optimizations 5.1–5.3 — Prompt caching, PostHog billing caps, Supabase spend cap, batch-resume-scorer EF + queue shimmer/poll, fly.toml auto-stop, annual billing toggle + Stripe Price, POSTHOG_MIGRATION_READY.md. | 🔲 | |
| FB-TRIAL-001-S7 | PostHog Events + Inline Nudges + QA — All 22 analytics events, 7 contextual inline nudges, upgrade page annual toggle, end-to-end testing. | 🔲 | |
