...
- **CRON-COST-OPT: COMPLETE** — all 12 spec items implemented and deployed. Score-on-demand was already handled by existing client flows (scoreResumeAI on resume upload, readiness rebuild on filter save). Onboarding completion trigger wired in app.js updateOnboardingStep().
